...
#                 w_ir = iou_ir / (iou_h + iou_ir)
        ...
        w_ir = torch.tensor(w_ir).cuda(1)
        w_h = torch.tensor(w_h).cuda(1)
        img = torch.tensor(img).cuda(1)
        ir = torch.tensor(ir).cuda(1)
        h = torch.tensor(h).cuda(1)
...
class AdaCh(nn.Module):  # Hunet
    ...
    def forward(self, img, gt):
        ...
        GT_b = w.bin_GT(gt).cuda(1)
        IR_b = w.bin_IR(ir).cuda(1)
        H_b = w.bin_H(h).cuda(1)
